...
function setup() {
	...
	background(255);
	const numPoints = floor((width + gap) / (barWidth + gap));
	array.length = 0;
	for (let i = 0; i < numPoints; i++) {
		array[i] = random(0, 1);
	}
	const graphWidth = numPoints * barWidth + (numPoints - 1) * gap;
	offsetLeft = floor((width - graphWidth) / 2);
	sorter = algorithms[select.value](array);
}
...
function draw() {
	...
	for (let i = 0; i < array.length; i++) {
		rect(offsetLeft + i * (barWidth + gap), height, barWidth, -array[i] * (height - 1));
	}
	if (playing) {
...
function* mergeSort(A, start = 0, stop = A.length - 1) {
	if (start < stop) {
		const split = floor((start + stop) / 2);
